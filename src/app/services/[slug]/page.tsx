...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  return (
    <>
      ...
      {/* Urgent banner for storm/leak repairs */}
      {service.urgent && (
        <aside aria-label="Emergency response notice" className="border-b border-urgent-700 bg-urgent-600 text-white">
          <div className="shell flex flex-wrap items-center justify-between gap-4 py-3.5">
            <p className="flex items-center gap-2.5 text-sm font-medium">
              <AlertIcon className="h-5 w-5 shrink-0" />
              <span>
                <strong>Need make-safe right now?</strong> Active water ingress or storm damage
                attended within 4 hours, 24/7 across the Brisbane southside.
              </span>
            </p>
            <a
              href={business.phone.emergency.href}
              className="btn bg-white py-1.5 px-4 text-sm font-bold text-urgent-700 hover:bg-cream"
            >
              <PhoneIcon className="h-4 w-4" />
              Call 24/7: {business.phone.emergency.display}
            </a>
          </div>
        </aside>
      )}
